...
def get_all_questions():
    ...
    try:
        ...
        questions = requests.post(
            f"http://{BACKEND_URL}/api/orange/questions",
            json={}
        ).json().get("question")
        return questions
...
def get_result():
    ...
    try:
        mbti=requests.post(
            f"http://{BACKEND_URL}/api/orange/result",
...
def clear():
    ...
    st.session_state.all_questions_loaded = False
    requests.post(
        f"http://{BACKEND_URL}/api/orange/clear",
        json={}
    )
    ...
    if not st.session_state.test_completed:
        ...
        for i in range(start_idx, end_idx):
            ...
            selected = st.radio(
                question[0],
                options,
                index=st.session_state.selected_options.get(i, 1),
                key=f"question_{i}"
            )
        ...
        career_recommendations = requests.post(
            url=f"http://{BACKEND_URL}/api/orange/seek",
